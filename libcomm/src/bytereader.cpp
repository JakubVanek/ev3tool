...
namespace ev3loader {
    ...
    bytereader::bytereader(const bytearray &data) : m_window(const_cast<bytearray &>(data)) {}

    bytereader::bytereader(buffer_view<> view) : m_window(view) {}
    ...
    void bytereader::finalize() const {
        if (size() != consumed()) {
